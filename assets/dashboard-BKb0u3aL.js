import{v as u,s as c}from"./auth-BWDIw5O6.js";import{S as v}from"./sweetalert2.esm.all-8-IS4QHV.js";import{b as p,r as x,s as g,h as m}from"./header-Dw_q0Stu.js";async function b(){var o;const e=await u();if(!e)return;p("dashboard"),x(e),g("Dashboard");const a=((o=e.user_metadata)==null?void 0:o.full_name)||e.email.split("@")[0],t=new Date().getHours(),n=t<12?"Bom dia":t<18?"Boa tarde":"Boa noite";document.getElementById("boas-vindas").innerHTML=`
    ...
    </div>`,await f(),document.body.style.visibility="visible"}async function f(){try{const[{count:e},{count:a},{count:t},{count:n},{data:o},{data:r},{data:i}]=await Promise.all([c.from("pratos").select("*",{count:"exact",head:!0}),c.from("pratos").select("*",{count:"exact",head:!0}).eq("disponivel",!0),c.from("categorias").select("*",{count:"exact",head:!0}),c.from("categorias").select("*",{count:"exact",head:!0}).eq("ativo",!0),c.from("pratos").select("id, nome, preco, disponivel, emoji, categorias(nome)").order("created_at",{ascending:!1}).limit(5),c.from("categorias").select("id, nome, descricao, ativo").eq("ativo",!0).order("nome").limit(6),c.from("pratos").select("nome, preco, disponivel, categorias(nome)")]);h(e,a,t,n),y(i||[]),k(o||[]),w(r||[]),M(i||[])}catch(e){v.fire({icon:"error",title:"Erro ao carregar dados",text:e.message})}}function h(e,a,t,n){const o=[{label:"Total de Pratos",valor:e,icon:"book-open",cor:"bg-orange-500"},{label:"Pratos Disponíveis",valor:a,icon:"check-circle",cor:"bg-green-500"},{label:"Total de Categorias",valor:t,icon:"tag",cor:"bg-blue-500"},{label:"Categorias Ativas",valor:n,icon:"chart-bar",cor:"bg-purple-500"}];document.getElementById("cards").innerHTML=o.map(r=>`
