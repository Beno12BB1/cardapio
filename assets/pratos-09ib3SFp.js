import{s as c,v as h}from"./auth-BWDIw5O6.js";import{S as n}from"./sweetalert2.esm.all-BFGDy3mj.js";import{b as I,r as C,s as _,a as k}from"./header-BE-Yf_-c.js";const g=10;let r=1,m=1,p="",u="",f="";const v=o=>(o||"").replace(/'/g,"\\'");async function P(){const o=await h();o&&(I("pratos"),C(o),_("Pratos"),k(),await T(),document.getElementById("btn-novo").addEventListener("click",()=>E()),document.getElementById("form-busca").addEventListener("submit",t=>{t.preventDefault(),p=document.getElementById("input-busca").value.trim(),u=document.getElementById("filtro-categoria").value,f=document.getElementById("filtro-disponivel").value,r=1,l()}),document.getElementById("btn-limpar").addEventListener("click",()=>{document.getElementById("input-busca").value="",document.getElementById("filtro-categoria").value="",document.getElementById("filtro-disponivel").value="",p="",u="",f="",r=1,l()}),document.getElementById("btn-anterior").addEventListener("click",()=>{r>1&&(r--,l())}),document.getElementById("btn-proxima").addEventListener("click",()=>{r<m&&(r++,l())}),await l(),document.body.style.visibility="visible")}async function T(){const{data:o}=await c.from("categorias").select("id, nome").eq("ativo",!0).order("nome"),t=document.getElementById("filtro-categoria");o&&o.forEach(e=>{const a=document.createElement("option");a.value=e.id,a.textContent=e.nome,t.appendChild(a)})}async function l(){const o=(r-1)*g;let t=c.from("pratos").select("id, nome, descricao, preco, tempo_preparo, disponivel, emoji, categorias(id, nome)",{count:"exact"}).order("nome");p&&(t=t.ilike("nome",`%${p}%`)),u&&(t=t.eq("categoria_id",u)),f&&(t=t.eq("disponivel",f==="true")),t=t.range(o,o+g-1);const{data:e,count:a,error:s}=await t;if(s){n.fire({icon:"error",title:"Erro",text:s.message});return}m=Math.max(1,Math.ceil((a||0)/g)),j(e||[]),document.getElementById("info-pagina").textContent=`Página ${r} de ${m}`,document.getElementById("btn-anterior").disabled=r===1,document.getElementById("btn-proxima").disabled=r===m}function j(o){const t=document.getElementById("tbody");if(!o.length){t.innerHTML=`<tr><td colspan="6" class="px-4 py-10 text-center text-slate-400">
      Nenhum prato encontrado.</td></tr>`;return}t.innerHTML=o.map(e=>{var d;const a=e.disponivel?'<span class="badge-disponivel">Disponível</span>':'<span class="badge-indisponivel">Indisponível</span>',s=e.disponivel?`<button onclick="window._toggle('${e.id}',false,'${v(e.nome)}')" class="btn-danger text-xs px-3 py-1">Tirar</button>`:`<button onclick="window._toggle('${e.id}',true,'${v(e.nome)}')"  class="btn-success text-xs px-3 py-1">Disponibilizar</button>`,i=e.tempo_preparo?`${e.tempo_preparo} min`:"—";return`
      <tr class="table-row">
        <td class="px-4 py-3">
