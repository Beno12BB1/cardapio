import{v as u,s as c}from"./auth-BWDIw5O6.js";import{S as v}from"./sweetalert2.esm.all-Dx-T5OB6.js";import{b as p,r as x,s as b,h as m}from"./header-25cIYKJM.js";async function g(){var o;const e=await u();if(!e)return;p("dashboard"),x(e),b("Dashboard");const a=((o=e.user_metadata)==null?void 0:o.full_name)||e.email.split("@")[0],t=new Date().getHours(),n=t<12?"Bom dia":t<18?"Boa tarde":"Boa noite";document.getElementById("boas-vindas").innerHTML=`
    <div>
    ...
    </div>`,await f(),document.body.style.visibility="visible"}async function f(){try{const[{count:e},{count:a},{count:t},{count:n},{data:o},{data:r},{data:i}]=await Promise.all([c.from("pratos").select("*",{count:"exact",head:!0}),c.from("pratos").select("*",{count:"exact",head:!0}).eq("disponivel",!0),c.from("categorias").select("*",{count:"exact",head:!0}),c.from("categorias").select("*",{count:"exact",head:!0}).eq("ativo",!0),c.from("pratos").select("id, nome, preco, disponivel, emoji, categorias(nome)").order("created_at",{ascending:!1}).limit(5),c.from("categorias").select("id, nome, descricao, ativo").eq("ativo",!0).order("nome").limit(6),c.from("pratos").select("nome, preco, disponivel, categorias(nome)")]);h(e,a,t,n),w(i||[]),$(o||[]),k(r||[]),y(i||[])}catch(e){v.fire({icon:"error",title:"Erro ao carregar dados",text:e.message})}}function h(e,a,t,n){const o=[{label:"Total de Pratos",valor:e,icon:"book-open",cor:"bg-orange-500"},{label:"Pratos Disponíveis",valor:a,icon:"check-circle",cor:"bg-green-500"},{label:"Total de Categorias",valor:t,icon:"tag",cor:"bg-blue-500"},{label:"Categorias Ativas",valor:n,icon:"chart-bar",cor:"bg-purple-500"}];document.getElementById("cards").innerHTML=o.map(r=>`
    <div class="card flex items-center gap-4">
      ...
      </div>`}).join("")}function k(e){const a=document.getElementById("lista-categorias");if(!e.length){a.innerHTML='<p class="text-slate-400 text-sm">Nenhuma categoria ativa.</p>';return}a.innerHTML=e.map(t=>`
    ...
    </div>`).join("")}function y(e){var o;const a={};e.forEach(r=>{var l;const i=((l=r.categorias)==null?void 0:l.nome)||"Sem categoria";a[i]=(a[i]||0)+1});const t=Object.entries(a).sort(([,r],[,i])=>i-r),n=((o=t[0])==null?void 0:o[1])||1;document.getElementById("grafico-categorias").innerHTML=t.length?t.map(([r,i])=>{const l=Math.round(i/n*100);return`
          <div>
